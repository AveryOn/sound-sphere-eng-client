...
export function useApi(apiConfig?: UseApiConfig) {
  ...
  const GET = async <R = unknown>(
    ...
    try {
      ...
      if (apiConfig?.logger?.levels?.includes('ERROR')) {
        console.error(formatApiMsg(url, 'GET', 'ERROR'), status, err)
      }
      throw err
    }
  }
  ...
  const POST = async <R = unknown>(
    ...
    try {
      ...
      const { status } = err as AxiosError
      if (apiConfig?.logger?.levels?.includes('ERROR')) {
        console.error(formatApiMsg(url, 'POST', 'ERROR'), status, err)
      }
      throw err
    }
  }
  ...
  const PUT = async <R = unknown>(
    ...
    try {
      ...
      const { status } = err as AxiosError
      if (apiConfig?.logger?.levels?.includes('ERROR')) {
        console.error(formatApiMsg(url, 'PUT', 'ERROR'), status, err)
      }
      throw err
    }
  }
  ...
  const PATCH = async <R = unknown>(
    ...
    try {
      ...
      const { status } = err as AxiosError
      if (apiConfig?.logger?.levels?.includes('ERROR')) {
        console.error(formatApiMsg(url, 'PATCH', 'ERROR'), status, err)
      }
      throw err
    }
  }
  ...
  const DELETE = async <R = unknown>(
    ...
    try {
      ...
      const { status } = err as AxiosError
      if (apiConfig?.logger?.levels?.includes('ERROR')) {
        console.error(formatApiMsg(url, 'DELETE', 'ERROR'), status, err)
      }
      throw err
    }
  }
